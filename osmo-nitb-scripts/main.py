...
def configure(gprs, sip, interface, config_path="/etc/osmocom"):
    ...
    subprocess.call("cp -f {0} {1}".format(app_dir+"/configs/osmo-trx.cfg", config_path+"/osmo-trx-uhd.cfg"), shell=True)
...
def run(gprs, sip):
    services = ["osmo-nitb.service", "osmo-trx-uhd.service", "osmo-bts-trx.service"]
    if gprs:
